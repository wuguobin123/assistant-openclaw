...
describe("models-config", () => {
  ...
  it("skips writing models.json when no env token or profile exists", async () => {
    await withTempHome(async (home) => {
      ...
      const previousSiliconflow = process.env.SILICONFLOW_API_KEY;
      ...
      try {
        ...
        if (previousSiliconflow === undefined) delete process.env.SILICONFLOW_API_KEY;
        else process.env.SILICONFLOW_API_KEY = previousSiliconflow;
  ...
  it("adds siliconflow provider when SILICONFLOW_API_KEY is set", async () => {
    await withTempHome(async () => {
      vi.resetModules();
      const prevKey = process.env.SILICONFLOW_API_KEY;
      process.env.SILICONFLOW_API_KEY = "sk-siliconflow-test";
      try {
        const { ensureOpenClawModelsJson } = await import("./models-config.js");
        const { resolveOpenClawAgentDir } = await import("./agent-paths.js");
        ...
        expect(parsed.providers.siliconflow?.baseUrl).toBe("https://api.siliconflow.cn/v1");
        expect(parsed.providers.siliconflow?.apiKey).toBe("SILICONFLOW_API_KEY");
        const ids = parsed.providers.siliconflow?.models?.map((model) => model.id);
        expect(ids).toContain("Pro/moonshotai/Kimi-K2.5");
      } finally {
        if (prevKey === undefined) delete process.env.SILICONFLOW_API_KEY;
        else process.env.SILICONFLOW_API_KEY = prevKey;
      }
    });
  });
