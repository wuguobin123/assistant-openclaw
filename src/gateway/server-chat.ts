...
export function createAgentEventHandler({
  ...
  const emitChatFinal = (
    ...
    if (jobState === "done") {
      ...
      nodeSendToSession(sessionKey, "chat", payload);
      if (text && sessionKey && !shouldSuppressHeartbeatBroadcast(clientRunId)) {
        const { cfg, entry } = loadSessionEntry(sessionKey);
        void forwardWebchatFinal({
          cfg,
          text,
          sessionKey,
          sessionChannel: entry?.lastChannel,
          log: logGateway,
        }).catch((err) => {
          logGateway?.warn(`webchat forward failed: ${formatForLog(err)}`);
        });
      }
      return;
    }
