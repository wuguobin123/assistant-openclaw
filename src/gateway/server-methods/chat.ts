...
import { forwardWebchatFinal } from "../webchat-forward.js";
      ...
      void dispatchInboundMessage({
        ...
        .then(() => {
          if (!agentRunStarted) {
            ...
            if (combinedReply) {
              const latest = loadSessionEntry(p.sessionKey);
              latestCfg = latest.cfg;
            ...
            if (combinedReply) {
              void forwardWebchatFinal({
                cfg: latestCfg,
                text: combinedReply,
                sessionKey: p.sessionKey,
                sessionChannel: latestEntry?.lastChannel ?? entry?.lastChannel ?? "webchat",
                log: context.logGateway,
              }).catch((err) => {
                context.logGateway.warn(`webchat forward failed: ${formatForLog(err)}`);
              });
            }
          }
